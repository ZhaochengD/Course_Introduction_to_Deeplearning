...
class CharacterPredictor(object):
    ...
    def forward(self, x, h):
        ...
        nexth = self.rnn.forward(x,h)
        logit = self.projection.forward(nexth)
        return logit, nexth
...
def inference(net, inputs):
    ...
    h0 = np.zeros((net.rnn.h), dtype=float)
    logits_li = []
    for t in range(inputs.shape[0]):
        logits, h0 = net.forward(inputs[t], h0)
        logits_li.append(logits)
    return np.stack(logits_li, axis = 0)
